...
func processMove(move: Character, currentKey: KeyPadPosition) -> KeyPadPosition
{
    ...
    case "U":
        ...
        case .Four, .Five, .Six, .Seven, .Eight, .Nine:
            return KeyPadPosition(rawValue: currentKey.rawValue - 3)!
        default: break
        }
        break
    ...
    case "L":
        switch currentKey
        {
        case .Two, .Three, .Five, .Six, .Eight, .Nine:
    ...
    case "R":
        switch currentKey
        {
        case .One, .Two, .Four, .Five, .Seven, .Eight:
            return KeyPadPosition(rawValue: currentKey.rawValue + 1)!
        default: break
        }
        break
...
for instructionsLine in input
{
    ...
    print("found number: \(currentKey.rawValue)")
    combination.append(currentKey.rawValue)
}
